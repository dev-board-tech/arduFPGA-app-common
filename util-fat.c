...
void loadAppPath() {
	char appPath[64];
	strcpy_P(appPath, PSTR("SD1:"));
	f_chdir(appPath);
	FRESULT file_status = f_open(&filObject, "current.txt", FA_READ);
	if(file_status == FR_OK) {
		f_gets(appPath, sizeof(appPath) - 4, &filObject);
		f_close(&filObject);
		uint16_t len = moveString(appPath, appPath + 4);
		char *ptr = appPath + len;
		for(; ptr > appPath && *ptr-- != '/'; ) {}
		ptr[1] = 0;
		memcpy_P(appPath, PSTR("SD1:"), 4);
		f_chdir(appPath);
	}
}

uint16_t moveString(char *from, char *to) {
	uint16_t len = strlen((char *)from);
	if(from < to) {
		for (uint16_t cnt = len + 1; cnt > 0; cnt--)
			to[cnt - 1] = from[cnt - 1];
	} else if(from > to) {
		for (uint16_t cnt = 0; cnt < len + 1; cnt++)
			to[cnt] = from[cnt];
	}
	return strlen(to);
}
