...
#ifndef USE_FFT_TABLES
    unsigned int i;
	sintable = (float *)malloc((FFT_BUFFER_SIZE / 2) * sizeof(float));
	costable = (float *)malloc((FFT_BUFFER_SIZE / 2) * sizeof(float));
	bitReverse = (unsigned long *)malloc(FFT_BUFFER_SIZE * sizeof(unsigned long));
#endif
