...
void stl_get_triangle(_3d_points *triangle_bounds, unsigned char *array, unsigned long triangle_nr)
{

	unsigned char *triangle = (((unsigned char*)array) + 84 + (50 * triangle_nr));
	float tmp;
...
unsigned long stl_get_nr_of_triangles(unsigned char *array)
{
	unsigned long tmp;
	memcpy(&tmp, (void*)(array + (sizeof(stl_header) - sizeof(unsigned long))), sizeof(unsigned long));
	return tmp;
}
